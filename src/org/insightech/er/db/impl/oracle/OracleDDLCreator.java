...
public class OracleDDLCreator extends DDLCreator {
    ...
    protected String getColulmnDDL(final NormalColumn normalColumn) {
        ...
        if (OracleDBManager.VERSION_12C && normalColumn.isAutoIncrement()) {
        	// Save the not null setting
        	boolean notNull = normalColumn.isNotNull();
        	normalColumn.setNotNull(false);
        	
        	// Generate the DDL and reset the not null value
        	ddl.append(super.getColulmnDDL(normalColumn));
        	normalColumn.setNotNull(notNull);
        	
            ddl.append(" GENERATED BY DEFAULT ON NULL AS IDENTITY ");
